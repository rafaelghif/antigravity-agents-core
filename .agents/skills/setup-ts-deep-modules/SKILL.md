...
description: >-
  Configures dependency-cruiser in a TypeScript project to enforce deep module boundaries with hidden internals and explicit entry points.
  Use when the user asks to enforce architectural module boundaries or set up dependency-cruiser in TypeScript.
---
...
For the vocabulary (deep module, interface, seam, depth), inspect "[codebase-design](../codebase-design/SKILL.md)" via `view_file` and use its language throughout.
...
Then add a **context pointer** to it from the repo's agent-instructions file ([AGENTS.md](../../../AGENTS.md)). One line is enough, e.g. `Packages are deep modules: see src/packages/README.md before adding or importing one.` This is what makes an agent discover the boundary rule instead of tripping over it.

**Done when:** `<packages-root>/README.md` exists and discourages barrels, and the repo's `AGENTS.md` links to it.
