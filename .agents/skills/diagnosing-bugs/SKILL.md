...
10. **HITL script (bash or PowerShell).** Last resort. If a human must click, drive _them_ with `scripts/hitl-loop.template.sh` (POSIX) or `scripts/hitl-loop.template.ps1` (Windows) so the loop is still structured. Captured output feeds back to you.
